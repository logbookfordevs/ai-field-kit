...
test("legacy built-in references migrate without changing custom sources, pins, or policy", () => {
  ...
  assert.equal(result.defaultsSource, "logbookfordevs/ai-field-kit-catalog");
  assert.deepEqual(result.favoriteSources, ["https://github.com/logbookfordevs/ai-field-kit-catalog", "acme/custom"]);
  ...
  assert.match(result.items[3].source, /ai-field-kit-catalog\/main\/rules/);
  ...
  assert.equal(migratedCatalogSource("https://github.com/logbookfordevs/ai-field-kit/tree/main/packages/afk/catalog"), "https://github.com/logbookfordevs/ai-field-kit-catalog/tree/main/afk/catalog");
});
...
test("cached legacy sources work immediately and migrate through planned writes without fetching", async () => {
  ...
  assert.equal(readRememberedDefaultsSource({ homeDir, manifestLocal: false }), "logbookfordevs/ai-field-kit-catalog");
  assert.equal(loadSkillManifest({ homeDir }).items[0]?.source, "https://github.com/logbookfordevs/ai-field-kit-catalog");
...
test("skill updates migrate only selected legacy lock entries and preserve a backup", async () => {
  ...
  assert.equal(result.skills.selected.source, "logbookfordevs/ai-field-kit-catalog");
  assert.equal(result.skills.selected.sourceUrl, "https://github.com/logbookfordevs/ai-field-kit-catalog");
