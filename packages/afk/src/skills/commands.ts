...
import { loadSkillCatalog, moveGlobalSkill, renameGlobalSkill, type SkillRecord } from "./catalog.js";
...
type SkillCommandName = "list" | "show" | "disable" | "enable" | "rename" | "categorize";
...
function runSkillsList(runtime: Runtime, options: CliOptions): number {
  ...
  if (options.skillsJson) {
    runtime.io.stdout(JSON.stringify(snapshot.records, null, 2));
    return 0;
  }

  if (snapshot.records.length === 0) {
    runtime.io.stdout(renderSkillList(snapshot.records, snapshot.categorization));
    return 0;
  }
...
async function runSkillsRename(folder: string | undefined, displayName: string, runtime: Runtime, options: CliOptions): Promise<number> {
  ...
  const record = folder
    ? findSkillRecord(snapshot.records, folder)
    : await promptSkillRecord(snapshot.records, "Select a global skill to rename:");
