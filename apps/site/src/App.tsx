...
import { AgentRouting } from '@/components/AgentRouting.tsx';
...
const paths = [
  ...
  { id: 'project', label: 'Project catalog', title: 'Commit the choices with the project.', text: 'Import the default Atlas catalog through shadcn, then preview project-local setup. Distribution and setup ownership stay explicit.', command: 'pnpm dlx shadcn@latest add logbookfordevs/ai-field-kit-catalog/afk-catalog\nafk setup --local --dry-run', facts: ['Repo-local catalog', 'Reviewable defaults', 'Explicit ownership'] },
];
const pieces = [
...
const HarnessIcon = ({ className }: { className?: string }) => <span className={`${className} harness-icon`} aria-hidden="true">+</span>;
const agents = [
...
function CopyButton({ value, onResult, compact = false }: { value: string; onResult: (message: string) => void; compact?: boolean }) {
...
function Command({ value, label, onResult }: { value: string; label: string; onResult: (message: string) => void }) {
...
function App() {
  ...
  useEffect(() => {
    document.title = 'AI Field Kit by Logbook for Devs';
  ...
  const select = (id: string) => { setActive(id); announce(`${paths.find((item) => item.id === id)?.label} selected`); };
  ...
  return <div className="app" data-lfd-recipe="ocean">
    ...
    <header><div className="nav-shell">
      <SiteLink className="brand" href="#top" aria-label="AI Field Kit home"><span>AFK</span>AI Field Kit</SiteLink>
      <nav aria-label="Primary navigation"><SiteLink href="#why">Why</SiteLink><SiteLink href="#kit">Kit</SiteLink><SiteLink href="#install">Install</SiteLink><SiteLink href="#agents">Agents</SiteLink><SiteLink href="/docs">Docs</SiteLink></nav>
      <SiteLink className="button button--small" href="#install">Try AFK</SiteLink>
    </div></header>
    <main id="main" tabIndex={-1}>
      <section className="hero" id="top"><div className="hero__grid">
        <div className="hero__copy" data-reveal>
          ...
          <div className="hero__actions"><SiteLink className="button" href="#install">Preview the setup</SiteLink><SiteLink className="text-link" href={repo} target="_blank" rel="noreferrer">Explore the repository<span className="sr-only"> (opens in a new tab)</span></SiteLink></div>
      ...
      <section className="closing"><div className="container closing__grid" data-reveal><h2>See the route.<br />Then run it.</h2><div className="closing__action"><span className="hand-note" aria-hidden="true">ready when you are</span><Command value={setupCommand} label="start safely" onResult={announce}/><SiteLink className="button button--light" href={repo} target="_blank" rel="noreferrer">Open GitHub</SiteLink></div></div></section>
    </main>
    <footer><div className="container footer-grid"><span>A tool from the <SiteLink href="https://logbookfordevs.com/" target="_blank" rel="noreferrer">Logbook for Devs</SiteLink><em>Charting the technical seas, one commit at a time.</em></span><div><SiteLink href={repo} target="_blank" rel="noreferrer">Repository</SiteLink><SiteLink href={`${repo}/blob/main/CHANGELOG.md`} target="_blank" rel="noreferrer">Changelog</SiteLink><SiteLink href="#top">Back to top</SiteLink></div></div></footer>
