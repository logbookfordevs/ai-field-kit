---
name: mscp-evaluation
description: Evaluate and compare solutions using MSCP (Maintainability, Strategy, Clarity, Performance). Use when the user asks “which approach is better?”, wants trade-off analysis, scoring, a structured critique of code/architecture/PRs, or a decision record that is explicit about trade-offs and future constraints.
---

# MSCP Evaluation

MSCP is a language for discussing software-quality trade-offs in real product teams.
...
3. Score **M/S/C/P from 1-5** with evidence (avoid vague claims like “clean” or “scalable”).
...
- **S — Strategy (product):** how well today’s choices solve the problem and keep future paths open.
- **C — Clarity (DX):** the speed and accuracy with which a competent engineer can construct a correct mental model of the decision scope (a function, module, PR, or system).
...
4. **Evaluate MSCP:** score each pillar 1-5 with a few evidence bullets.
...
- `references/OVERVIEW.md`: what MSCP is/isn’t, when to use it, and its naming context.
...
- `references/ANTI-PATTERNS.md`: common ways MSCP gets misused and how to avoid them.
