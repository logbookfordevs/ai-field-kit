...
Treat explicit "AFK workflow" or "feature workflow" requests as AFK orchestration: use `afk-compass` at the start and at phase changes, with `afk-execution-tracking` as the continuity surface. Treat explicit "AFK Turbo" requests as high-throughput GoalBuddy-backed execution routed by Compass.
