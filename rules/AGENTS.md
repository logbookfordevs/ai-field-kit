...
Treat explicit "AFK workflow" or "feature workflow" requests as AFK Flow: use `afk-compass` at the start and at phase changes, with `afk-execution-tracking` as the continuity surface. Treat explicit "AFK Sprint" or "AFK Turbo" requests as fast outcome execution routed by Compass.
